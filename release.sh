...
if egrep -q 'release|push|pull_request|workflow_dispatch|workflow_run|schedule|repository_dispatch' <<<"${GITHUB_EVENT_NAME}"; then
  echo "Event: ${GITHUB_EVENT_NAME}"
else
  echo -e "Unsupport event: ${GITHUB_EVENT_NAME}! \nSupport: release | push | pull_request | workflow_dispatch | workflow_run | schedule | repository_dispatch"
  exit 1
fi
